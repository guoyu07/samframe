...
class DBPDO
{
	...
	{/*{{{*/
		...
		$stmt = new DBStatment($this->_conn->prepare($sql));
		if(is_array($params)) {
			if(!empty($params)) {
				$i = 0;
		...
		if($this->_optimize && preg_match("/^select/i", $sql)) {
			...
			$debug = $this->_debug;
			$this->setDebug(true);
			DBExplainResult::draw($this->getAll("explain ".$this->getBindedSql($sql, $params)));
			$this->setDebug($debug);
	...
	{/*{{{*/
		...
		if(in_array(preg_replace("/\s{2,}/", " ", strtolower($sql)), array("begin", "commit", "rollback", "start transaction", "set autocommit=0", "set autocommit=1"))) {
			throw new DBException("为避免操作异常，请使用包装后的事务处理接口[startTrans, commit, rollback]");
		}

		if($this->_transaction) {
			if($this->_reconnected) {
				throw new DBException("数据库链接已丢失!");
			} else {
				try {
					...
					throw new DBException($e->errorInfo[2], $e->errorInfo[1]);
				}
			}
		} else {
			try {
	...
	{/*{{{*/
		...
		if(!$is_safe) {
			throw new DBException("SQL语句:[$sql],存在SQL注入漏洞隐患，请改用bind方式处理或关闭sql执行safe模式.");
		}
	...
	{/*{{{*/
		if(in_array($operate, array("insert", "replace", "update"))) {
			...
			if(empty($fields)) {
				throw new DBException("\$record 操作数据必须使用关联数组形式");
			}
		}
	...
	{/*{{{*/
		if($this->_transaction) {
			throw new DBException("之前开启的事务尚未结束，事务处理不能嵌套操作!");
		}
		...
		try {
			...
			throw new DBException($errorInfo[2], $errorInfo[1]);
		}
	...
	{/*{{{*/
		if(!$this->_transaction) {
			throw new DBException("之前开启的事务已经被提交或没有开启，请仔细查看事务处理过程中的操作语句!");
		}
