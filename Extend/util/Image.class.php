...
class Image {
     ...
     * @return string
     */
    static function buildImageVerify($randval = "",$length=4, $mode=1, $type='png', $width=90, $height=40, $verifyName='verify') {
        ...
        for ($i = 0; $i < $length; $i++) {
			...
			imagettftext ($im, 17,mt_rand(0,30), $i * 20 + 10, mt_rand(20, 35), imagecolorallocate($im, mt_rand(0, 200), mt_rand(0, 200), mt_rand(0, 200)), dirname(__FILE__) . '/ITCKRIST.TTF', $randval{$i} );
    ...
    // 中文验证码
    static function GBVerify($length=4, $type='png', $width=180, $height=50, $fontface='STHUPO.TTF', $verifyName='verify') {
        $code =  "高山流水";
        $im = imagecreatetruecolor($width, $height);
        $borderColor = imagecolorallocate($im, 100, 100, 100);                    //边框色
